...
//    public ResponseEntity<?>
    ...
    @PostMapping("/fare")
    public ResponseEntity<?> createFareDetails(@RequestBody FareRuleDTO fareRuleDTO){
        try {

            FareRuleDTO created = trainService.createFareDetails(fareRuleDTO);
            Optional<fareRuleSet> fareRuleSet = trainService.getFareRuleSetByName(created.getRuleSetName());

            List<fareClassRate> classRates=new ArrayList<>();
            for(FareRateDTO fr:fareRuleDTO.getFareRates()){
                Optional<fareClassRate> rate= trainService.getFareClassRateByClassTypeAndRuleSetId(fr.getClassType(), fareRuleSet.get().getRuleSetId());
                rate.ifPresent(classRates::add);
            }

            Map<String,Object> responseBody = new HashMap<>();

            responseBody.put("fareRuleSet", fareRuleSet.orElse(null));
            responseBody.put("fareClassRate", classRates);
            ...
            return error(HttpStatus.INTERNAL_SERVER_ERROR, "Unexpected error creating fare details", ex);
        }
    }
}