...
//
    ...
    @Transactional
    public FareRuleDTO createFareDetails(FareRuleDTO fareRuleDTO) {


           Optional<fareRuleSet> existingRS=fareRuleSetRepo.findByRuleSetName(fareRuleDTO.getRuleSetName());

           if(existingRS.isPresent()) {
                throw new IllegalArgumentException("Fare Rule Set with name '" + fareRuleDTO.getRuleSetName() + "' already exists");
           }

            List<fareClassRate> classRates=new ArrayList<>();

            fareRuleSet tempRS=new fareRuleSet();

            tempRS.setRuleSetName(fareRuleDTO.getRuleSetName());
            tempRS.setEffectiveFrom(fareRuleDTO.getEffectiveFrom());
            tempRS.setEffectiveTo(fareRuleDTO.getEffectiveTo());
            tempRS.setCurrency(fareRuleDTO.getCurrency());
            tempRS.setDescription(fareRuleDTO.getDescription());
            tempRS.setCreatedAt(new Date());
            tempRS.setUpdatedAt(new Date());

            Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
            String currentPrincipalName = (authentication != null) ? authentication.getName() : "system";
            tempRS.setCreatedBy(currentPrincipalName);

            fareRuleSet savedRS=fareRuleSetRepo.save(tempRS);

            for(FareRateDTO fr:fareRuleDTO.getFareRates()){
                fareClassRate tempCR=new fareClassRate();
                tempCR.setBaseFare(fr.getBaseFare());
                tempCR.setRatePerKm(fr.getRatePerKm());
                tempCR.setClassType(fr.getClassType());
                tempCR.setMinFare(fr.getMinFare());
                tempCR.setCreatedAt(new Date());
                tempCR.setUpdatedAt(new Date());
                tempCR.setFareRuleSetId(savedRS.getRuleSetId());

                classRates.add(tempCR);
            }

            fareClassRateRepo.saveAll(classRates);

            return fareRuleDTO;
    }

    public Optional<fareClassRate> getFareClassRateByClassTypeAndRuleSetId(String classType, Integer ruleSetId) {
        return fareClassRateRepo.findByClassTypeAndFareRuleSetId(classType, ruleSetId);
    }

    public Optional<fareRuleSet> getFareRuleSetByName(String ruleSetName) {
        return fareRuleSetRepo.findByRuleSetName(ruleSetName);
    }
